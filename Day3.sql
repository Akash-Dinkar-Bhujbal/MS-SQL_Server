...
----------------------------------------------------------------------
-- If Road is present in Address Count++
...
select CHARINDEX('Rd',address)  from CUSTOMERS --CHARINDEX start with 'Rd'
select CHARINDEX('Road',address)  from CUSTOMERS --CHARINDEX start with 'Road'
-------------------------------------------------------------------------
DROP TABLE PRODUCT
...
--- If 'b/B' is not present then print
	select Description from product
	  where patindex('%[b,B]read%',description) =0
--- If 'b/B' is present then print
select Description from product
	  where patindex('%[b,B]read%',description) >0
--select Description from product where patindex('%[b,B]read%',description) <0
--<0 is not logical, that's why not printing anything
-------------------------------------------------------------------------------
SELECT * FROM CUSTOMERS
-- Find DOMAINNAME of Email
SELECT a.CONTACTNAME,	--contactname
             a.email,	--email
       Substring (	a.email, 
					Charindex( '@', email ) + 1, 	-- Start Position
					Len(email)						-- End Position.
				) AS DOMAINNAME						-- Alise name for column
	FROM   CUSTOMERS a								-- Alise name for table
-----------------------------------------
-- Find how many emails are present in that Domain
SELECT Count(*) 'Count', 
       Substring (	a.email, 
					Charindex( '@', email ) + 1, 
					Len(email)
...
--0 indicates weak or no similarity between the SOUNDEX values. 4 indicates strong similarity or identically SOUNDEX values.
	...
	SELECT	SOUNDEX('javascript') soundex_javascript, 
			SOUNDEX('c#') soundex_c#,
...
---------***INSERT TRIGGER
	...
	END
DROP TABLE EMP
CREATE TABLE EMP(NO INT,SALARY INT)
--USING TRIGGER 
INSERT INTO EMP VALUES(1,3200)
-----------------------------***UPDATE TRIGGER
CREATE TRIGGER T5
ON EMP
FOR UPDATE
...
IF (@A > 2000)
BEGIN
PRINT 'THE AVERAGE SALARY CAN NOT BE MORE THAN 2000'
